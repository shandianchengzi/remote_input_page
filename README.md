...
页面下方提供虚拟触摸板区域和鼠标左右键按钮：

- **触摸板区域**：手指在灰色虚线区域内滑动，电脑光标会实时跟随移动。采用 40ms 节流（每秒 25 帧）避免请求风暴
- **轻触触摸板** = 鼠标左键单击（300ms 内松开且无明显位移）
- **长按拖动**：按住超过 300ms 后进入拖拽模式（按住左键），移动手指拖动对象，松开释放
- **左键单击**：红色按钮，点击即可在电脑光标位置触发一次鼠标左键点击
- **右键单击**：紫色按钮，点击即可在电脑光标位置触发一次鼠标右键点击
...
- 前后端通讯使用 JSON 格式，支持 `text`（文本）、`key`（单键）、`shortcut`（组合键）、`mouse_move`（鼠标移动）、`mouse_click`（鼠标点击）、`mouse_down`（按键按下）、`mouse_up`（按键释放）七种类型
